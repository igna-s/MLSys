...
struct SubgraphTensorInfo {
    ...
    struct TInfo {
        ...
        bool is_produced;
        bool skip_store;  // V19: produced AND consumed in subgraph → evaluator treats as ephemeral for store
    };
    ...
    const GraphInfo& gi)
{
    ...
    // Compute ephemeral tensors (same as v18: last_use check)
    ...
    for (size_t t : all_t_vec) {
        ...
        // V19: skip_store if produced AND consumed in subgraph AND not graph output
        // This matches evaluator's ephemeral definition for store cost only
...
static EvalResult eval_inner_precomp(
    ...
    for (auto& ti : si.tinfos) {
        ...
        // V19: skip store for tensors consumed within subgraph (evaluator treats as ephemeral)
    ...
    const std::vector<int>& lu, const GraphInfo& gi)
{
    ...
    // V19: Compute resident_full_size early (needed for memory-aware combos)
    ...
    // V18 memory-aware h candidates (nw proxy — kept for backward compat)
    ...
    if (si.has_mm && si.K_dim > 0) {
        ...
        // V19: k=4 and k=8 as safety nets for tight OOM
    ...
    // V19: Memory-aware combos — compute exact max_h for targeted w values
    // Only for w=outW (single-column case) and a few key w values where the
    // nw-proxy approach misses important configurations.
    if (si.has_mm && si.K_dim > 0 && !si.ws_mm_lhs.empty()) {
        // V19: Memory-aware combos for targeted w values.
        // Computing exact max_h per (w,k) captures OOM-boundary h values missed by gen().
        ...
        // Add fractions of outW/outH to cover intermediate w values
        // Only for small problems — too many combos cause 1-core timeouts on large N
        // N<32: b5(N=19) benefits, b9(N=32) doesn't benefit and needs timing margin
        if ((int)p.ops.size() < 32) {
    ...
    for (int i = 1; i <= N; ++i) {
        for (int j = std::max(0, i - W); j < i; ++j) {
            ...
            std::vector<size_t> sg_tensors_vec;
            {
                ...
                // (Pass-through retention disabled — v18 behavior)
            }
